...
def Dbar(x, eta=1.0, x0=0):
    y = np.zeros([len(x), 1])
    x = x - x0
    for ix in range(0, len(x)):
        y[ix] = (1 - (np.tanh(x[ix] / eta))**2.0) / (2.0 * eta) * (2.0 * eta)
    return y
# ----------------------------
nx = 5
...
rmsd = np.sqrt(np.sum((uIB[:xInd, 0] - uAnal[:xInd, 0])**2) / len(uAnal[:xInd])) / (np.max(np.abs(uAnal[:xInd])) -
